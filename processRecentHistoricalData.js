...
// Process recent historical workshop data (WoG 2020, 2019, WPSG 2020) and integrate with comprehensive database
import fs from 'fs';
...
console.log('🔄 Processing Recent Historical Workshop Integration (2019-2020)\n');

// Load all data
const facultyData = JSON.parse(fs.readFileSync(path.join(__dirname, 'src/data/facultyData.json'), 'utf8'));
const existingTeaching = JSON.parse(fs.readFileSync(path.join(__dirname, 'data/processed/teachingDataHistoricalComplete.json'), 'utf8'));

// Load new historical workshop data
const workshops = [
  { file: 'wog-2020.json', workshop: 'WoG', year: 2020 },
  { file: 'wog-2019.json', workshop: 'WoG', year: 2019 },
...
function getSpecialization(topic, workshop) {
  ...
  if (workshop === 'WoG') {
    ...
    } else if (topicLower.includes('marine') || topicLower.includes('phytoplankton') || topicLower.includes('ocean')) {
      return 'Marine Genomics';
    } else if (topicLower.includes('microbial ecology')) {
      return 'Microbial Ecology';
    } else if (topicLower.includes('visualization')) {
      return 'Data Visualization';
    } else if (topicLower.includes('evolutionary genomics')) {
      return 'Evolutionary Genomics';
    } else if (topicLower.includes('unix') || topicLower.includes('data') || topicLower.includes('computational') || topicLower.includes('statistics')) {
  ...
  if (workshop === 'WPSG') {
    if (topicLower.includes('population genomics') || topicLower.includes('population structure')) {
      return 'Population Genomics';
    ...
    } else if (topicLower.includes('speciation') || topicLower.includes('hybridization')) {
    ...
    } else if (topicLower.includes('multi-species')) {
      return 'Multi-species Coalescent';
    ...
    } else if (topicLower.includes('experimental design') || topicLower.includes('demography')) {
      return 'Experimental Design';
    } else if (topicLower.includes('likelihood') || topicLower.includes('bayesian') || topicLower.includes('probability')) {
...
console.log(`📋 Processing ${workshops.length} recent historical workshops:\n`);
...
Object.keys(facultyTeachingData).forEach(facultyId => {
  if (mergedTeachingData[facultyId]) {
    ...
    const newRecent = facultyTeachingData[facultyId].teaching;
    
    // Merge workshop histories
    Object.keys(newRecent.workshopsHistory).forEach(workshopType => {
      if (!existing.workshopsHistory[workshopType]) {
        existing.workshopsHistory[workshopType] = {};
      }
      Object.keys(newRecent.workshopsHistory[workshopType]).forEach(year => {
        existing.workshopsHistory[workshopType][year] = 
          newRecent.workshopsHistory[workshopType][year];
      });
    });
    
    // Update totals
    existing.totalSessions += newRecent.totalSessions;
    existing.lastTaught = Math.max(existing.lastTaught, newRecent.lastTaught);
    existing.firstTaught = Math.min(existing.firstTaught, newRecent.firstTaught);
    
    // Merge specializations
    const allSpecs = new Set([...existing.specializations, ...newRecent.specializations]);
    existing.specializations = Array.from(allSpecs);
    
    // Merge years active
    const allYears = new Set([...existing.yearsActive, ...newRecent.yearsActive]);
...
console.log(`\n📊 Recent Historical Integration Results (2019-2020):`);
...
console.log(`- Faculty with new recent historical teaching: ${Object.keys(facultyTeachingData).length}`);
...
const outputPath = path.join(__dirname, 'data/processed/teachingDataCompleteHistorical.json');
fs.writeFileSync(outputPath, JSON.stringify(mergedTeachingData, null, 2));

console.log(`\n💾 Saved complete historical teaching data to: ${outputPath}`);
console.log(`\n🎯 Complete Enhanced Historical Coverage:`);
...
console.log(`\n🚀 Ready to deploy enhanced comprehensive historical teaching database!`);