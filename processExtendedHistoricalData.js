...
// Process extended historical workshop data (2015-2018) and integrate with comprehensive database
import fs from 'fs';
...
console.log('🔄 Processing Extended Historical Workshop Integration (2015-2018)\n');

// Load all data
const facultyData = JSON.parse(fs.readFileSync(path.join(__dirname, 'src/data/facultyData.json'), 'utf8'));
const existingTeaching = JSON.parse(fs.readFileSync(path.join(__dirname, 'data/processed/teachingDataCompleteHistorical.json'), 'utf8'));

// Load new extended historical workshop data
const workshops = [
  { file: 'wog-2018.json', workshop: 'WoG', year: 2018 },
  { file: 'wog-2017.json', workshop: 'WoG', year: 2017 },
  { file: 'wog-2016.json', workshop: 'WoG', year: 2016 },
  { file: 'wog-2015.json', workshop: 'WoG', year: 2015 }
];
...
function getSpecialization(topic, workshop) {
  ...
  if (workshop === 'WoG') {
    ...
    } else if (topicLower.includes('epigenetics')) {
      return 'Epigenetics';
    } else if (topicLower.includes('pathogenomics')) {
...
console.log(`📋 Processing ${workshops.length} extended historical workshops:\n`);
...
Object.keys(facultyTeachingData).forEach(facultyId => {
  if (mergedTeachingData[facultyId]) {
    ...
    const newExtended = facultyTeachingData[facultyId].teaching;
    
    // Merge workshop histories
    Object.keys(newExtended.workshopsHistory).forEach(workshopType => {
      if (!existing.workshopsHistory[workshopType]) {
        existing.workshopsHistory[workshopType] = {};
      }
      Object.keys(newExtended.workshopsHistory[workshopType]).forEach(year => {
        existing.workshopsHistory[workshopType][year] = 
          newExtended.workshopsHistory[workshopType][year];
      });
    });
    
    // Update totals
    existing.totalSessions += newExtended.totalSessions;
    existing.lastTaught = Math.max(existing.lastTaught, newExtended.lastTaught);
    existing.firstTaught = Math.min(existing.firstTaught, newExtended.firstTaught);
    
    // Merge specializations
    const allSpecs = new Set([...existing.specializations, ...newExtended.specializations]);
    existing.specializations = Array.from(allSpecs);
    
    // Merge years active
    const allYears = new Set([...existing.yearsActive, ...newExtended.yearsActive]);
...
console.log(`\n📊 Extended Historical Integration Results (2015-2018):`);
...
console.log(`- Faculty with new extended historical teaching: ${Object.keys(facultyTeachingData).length}`);
...
const outputPath = path.join(__dirname, 'data/processed/teachingDataExtendedHistorical.json');
fs.writeFileSync(outputPath, JSON.stringify(mergedTeachingData, null, 2));

console.log(`\n💾 Saved extended historical teaching data to: ${outputPath}`);
console.log(`\n🎯 Complete Extended Historical Coverage:`);
...
console.log(`\n🚀 Ready to deploy extended historical teaching database (2015-2025)!`);